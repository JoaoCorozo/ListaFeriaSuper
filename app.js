...
{ id: "saldo", nombre: "Sal", categoria: "supermercado", unidadDefault: "kg", imagen: "img/super/sal.jpg" },
...
// Unidades
const UNIDADES_FRECUENTES = ["unidad", "kg", "litro", "bandeja", "paquete", "caja", "frasco"];
...
function abrirModalAgregar(producto) {
  ...
  if (
    ...
    modalUnidad.value = UNIDADES_FRECUENTES[0];
  }
...
function crearItemElemento(item, index) {
  ...
  selectUnidad.value = item.unidad || UNIDADES_FRECUENTES[0];
...
btnBorrarTodo.addEventListener("click", () => {
  ...
  if (!confirm("¿Seguro que quieres borrar toda la lista (todas las categorías)?")) return;
...
btnBorrarComprados.addEventListener("click", () => {
  ...
  if (!confirm("¿Borrar solo los productos comprados de esta categoría?")) return;
